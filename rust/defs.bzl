...
def rust_proto_library(name, deps, **args):
    ...
    Args:
        ...
        **args: other args passed to the rust_<backend>_proto_library targets.
    """
    ...
    native.alias(
        name = name,
        actual = select({
            "//rust:use_upb_backend": name + "_upb_backend",
            "//conditions:default": name + "_cpp_backend",
        }),
    )

    rust_upb_proto_library(name = name + "_upb_backend", deps = deps, **args)
    ...
    rust_cc_proto_library(name = name + "_cpp_backend", deps = [_cc_proto_name], **args)
